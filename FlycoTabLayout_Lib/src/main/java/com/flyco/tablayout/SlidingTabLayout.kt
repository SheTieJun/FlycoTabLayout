...
class SlidingTabLayout @JvmOverloads constructor(private val mContext: Context, attrs: AttributeSet? = null, defStyleAttr: Int = 0) : HorizontalScrollView(mContext, attrs, defStyleAttr), ViewPager.OnPageChangeListener {
    ...
    private fun obtainAttributes(context: Context, attrs: AttributeSet?) {
        ...
        mTextSelectSize = ta.getDimension(R.styleable.SlidingTabLayout_tl_textSelectSize, sp2px(14f).toFloat())
        ...
        for (i in 0 until tabCount) {
            ...
            if (tv_tab_title != null) {
                ...
                if (i == mCurrentTab) {
                    ...
                    tv_tab_title.animate().scaleX(fl).scaleY(fl).setDuration(180).start()
                }else{
                    tv_tab_title.animate().scaleX(1f).scaleY(1f).setDuration(180).start()
                }
    ...
    private fun updateTabSelection(position: Int) {
        for (i in 0 until tabCount) {
            ...
            if (isSelect) {
                val fl = mTextSelectSize / mTextSize
                tab_title.animate().scaleX(fl).scaleY(fl).setDuration(180).start()
            }else{
                tab_title.animate().scaleX(1f).scaleY(1f).setDuration(180).start()
            }
        }
    }
