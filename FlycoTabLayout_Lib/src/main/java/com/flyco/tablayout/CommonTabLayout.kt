...
class CommonTabLayout @JvmOverloads constructor(private val mContext: Context, attrs: AttributeSet? = null, defStyleAttr: Int = 0) : FrameLayout(mContext, attrs, defStyleAttr), ValueAnimator.AnimatorUpdateListener {
    ...
    /** 关联数据支持同时切换fragments  */
    fun setTabData(tabEntitys: ArrayList<CustomTabEntity>, fa: FragmentActivity, containerViewId: Int, fragments: ArrayList<Fragment>) {
        ...
        if (tipView != null) {
            ...
            if (!mIconVisible) {
                ...
                setMsgMargin(position, 0f,
                        (if (mIconGravity == Gravity.LEFT || mIconGravity == Gravity.RIGHT) 4 else 0).toFloat())
            }
    ...
    fun setMsgMargin(position: Int, leftPadding: Float, bottomPadding: Float) {
        ...
        if (tipView != null) {
            ...
            if (mIconGravity == Gravity.TOP || mIconGravity == Gravity.BOTTOM) {
                ...
                lp.topMargin = if (mHeight > 0) (mHeight.toFloat() - textHeight - iconH - margin).toInt() / 2 - dp2px(bottomPadding) else dp2px(bottomPadding)
            } else {
                lp.leftMargin = dp2px(leftPadding)
